...
When you are confident you can generate sensible random structures, run your search with `sbatch castep_search.job`. There are many other input flags you may want to use. Have a look at the output of `airss.pl -h`. Feel free to ask us about any settings which confuse you or about which you would like to know more! In particular, you may want to change `-mpinp`, which governs the parallelisation of the underlying CASTEP calculation. Make sure `-mpinp` is a factor of the number of cores you have available. The search may take some time. While the search runs, read on in this practical and prepare for your next steps.\
